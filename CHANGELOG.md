...
- **단계별 작업 보고서 저장** (`gurunote/reporting.py`, `app.py`, `gui.py`)
  - 성공/실패와 무관하게 단계 로그를 Markdown 파일로 즉시 누적 저장
  - 크래시/중단 상황에서도 이전 단계 진행 이력 추적 가능
- **작업 이력/결과 자동 저장 추가** (`gurunote/history.py`, `app.py`, `gui.py`)
  - 결과 마크다운을 기본 옵션으로 `history/`에 자동 저장
  - 앱 내 이력 화면에서 성공/실패 상태와 결과물/보고서를 재열람 가능
- **VibeVoice CPU 지연 안전장치** (`gurunote/stt.py`)
  - `auto` 모드에서 GPU/MPS가 없으면 AssemblyAI로 즉시 전환
  - `vibevoice` 강제 CPU 사용은 `VIBEVOICE_ALLOW_CPU=1`에서만 허용
