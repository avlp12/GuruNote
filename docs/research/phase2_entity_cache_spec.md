...
**자동 verify (NVIDIA GTC 25 segments):**
1. chunk 1 entity 추출 catch: `entity_cache` 가 비어 있던 상태에서 `Schneider Electric → 슈나이더 일렉트릭`, `Pankaj Sharma → 판카즈 샤르마` 등 추출.
2. chunk 2+ context inject catch: `_build_entity_cache_block` 출력이 chunk 2+ 의 prompt 영역에 포함.
3. chunk 2+ 출력에서 dict 영역 entity 의 한국어 표기 100% 정합.
4. dict 외부 신규 entity 의 첫 등장 영문 병기 정합.
...
### 8.1 Phase 1 Step 2 (chunk size 축소) 완료 후 진입 영역

**트랙 A 의 Step 2 (DEFAULT_CHUNK_CHAR_LIMIT 12000 → 6000) 완료 catch 후:**

1. Step 2 verify (NVIDIA GTC E2E):
   - 길이 미스매치 0건.
   - timestamp 완전성 100%.
   - 회귀 catch 5건의 어떤 영역이 chunk size 영향 catch 인지 verify.

2. Phase 2 진입 전 catch 부재 영역:
   - 회귀 1 (스키에더 vs 슈나이더): Step 2 chunk size 축소 영역으로 chunk 경계가 더 자주 발생 → 영향 verify 필요.
   - 회귀 2 (티파즈 hallucinate): Step 2 와 무관 추정 (LLM 의 chunk 내부 hallucinate).
...
| 시점 | 조건 |
|------|------|
| Phase 1 Step 2 완료 | DEFAULT_CHUNK_CHAR_LIMIT 6000 verify 통과 |
| Phase 1 Step 3 완료 (있다면) | research spec 결합 B 의 prompt 영역 |
| Phase 1 전체 verify 통과 | NVIDIA GTC E2E 길이 미스매치 0건 + timestamp 완전성 100% |
| Phase 2 진입 | 위 3개 조건 정합 후 |

**추정 시점:** Phase 1 trajectory 가 5/14 21:02 (Step 2 진입) 시작 영역이라, Step 2 + Step 3 verify 정합 시 Phase 2 진입은 5/15~5/16 영역 추정. 단 trajectory 의 회귀 catch 영역에 따라 변동.
